...
describe('Fragments generator - tag', function() {
  ...
  describe('Disable index page', function() {
    it('pagination enabled', function() {
      ...
      for (var i = 0, len = result.length; i < len; i++) {
        result[i].layout.should.eql(['fragments-tag']);
      }

      result[0].path.should.eql('fragments/tags/foo/');
      result[0].data.base.should.eql('fragments/tags/foo/');
      result[0].data.total.should.eql(2);
      result[0].data.current.should.eql(1);
      result[0].data.current_url.should.eql('fragments/tags/foo/');
      ...
      result[0].data.next_link.should.eql('fragments/tags/foo/2/');
      result[0].data.tag.should.eql('foo');

      result[1].path.should.eql('fragments/tags/foo/2/');
      result[1].data.base.should.eql('fragments/tags/foo/');
      result[1].data.total.should.eql(2);
      result[1].data.current.should.eql(2);
      result[1].data.current_url.should.eql('fragments/tags/foo/2/');
      result[1].data.posts.eq(0)._id.should.eql(posts[2]._id);
      result[1].data.prev.should.eql(1);
      result[1].data.prev_link.should.eql('fragments/tags/foo/');
      result[1].data.next.should.eql(0);
      result[1].data.next_link.should.eql('');
      result[1].data.tag.should.eql('foo');

      result[2].path.should.eql('fragments/tags/bar/');
      result[2].data.base.should.eql('fragments/tags/bar/');
      result[2].data.total.should.eql(1);
      result[2].data.current.should.eql(1);
      result[2].data.current_url.should.eql('fragments/tags/bar/');
    ...
    it('pagination disabled', function() {
      ...
      result[1].path.should.eql('fragments/tags/bar/');
      result[1].data.base.should.eql('fragments/tags/bar/');
      result[1].data.total.should.eql(1);
      result[1].data.current.should.eql(1);
      result[1].data.current_url.should.eql('fragments/tags/bar/');
    ...
    it('custom pagination_dir', function() {
      ...
      result.map(function(item) {
        return item.path;
      }).should.eql(['yo/tags/foo/', 'yo/tags/foo/2/', 'yo/tags/bar/']);
